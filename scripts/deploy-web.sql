...
CREATE TABLE `construct_record` (
	...
	`project_id` int(10) NOT NULL COMMENT '工程ID',
	`mirror_id` int(10) COMMENT '构建产生的镜像ID',
	`construct_start` datetime default NULL COMMENT '工程构建的开始时间点',
	`construct_end` datetime default NULL COMMENT '工程构建的结束时间点',
...
CREATE TABLE `deploy` (
	...
	`deploy_start` datetime default NULL COMMENT '部署的开始时间点',
	`deploy_end` datetime default NULL COMMENT '部署的结束时间点',
	`host_list` text default NULL COMMENT '机器列表',
	`mirror_list` varchar(500) default NULL COMMENT '镜像列表',
	`docker_config` text default NULL COMMENT 'docker config文件',
